...
public class Test_CWrittenToken {
  ...
  @Test
  public void testReadFromFileNoSourceDevice() {
    final String testResourcesPath = "test" + File.separator + "resources";
    final URL wtUrl = this.getClass().getClassLoader().getResource(testResourcesPath +
                File.separator + "tokens" + File.separator + "token_no_source.wt");

    File f = null;
    try {
      f = File.createTempFile("test_token", ".wt");
      FileUtils.copyURLToFile(wtUrl, f);

      CWrittenToken token = new CWrittenToken();
      String tokenName = token.readFromFile(f);

      assertEquals("f", tokenName);
      assertEquals(2, token.nStrokes());
      assertEquals(28, token.getStroke(0).nPoints());
      assertEquals(10, token.getStroke(1).nPoints());
    } catch (IOException e) {
  ...
  @Test
  public void testReadFromFileWithSourceDeviceAndUnicodeToken() {
    final String testResourcesPath = "test" + File.separator + "resources";
    final URL wtUrl = this.getClass().getClassLoader().getResource(testResourcesPath +
    ...
    try {
      ...
      assertEquals("不", tokenName);
      assertEquals(4, token.nStrokes());
      assertEquals(10, token.getStroke(0).nPoints());
      assertEquals(11, token.getStroke(1).nPoints());
      assertEquals(14, token.getStroke(2).nPoints());
      assertEquals(6, token.getStroke(3).nPoints());
    } catch (IOException e) {
      fail("Failed due to IOException: " + e.toString());
    } finally {
    	f.delete();
